...
import type { Feedback } from '../features/apiSlice'
...
const Main: React.FC = () => {
  ...
  const comments = (data as unknown as { feedbacks: Feedback[] })?.feedbacks || []

  const normalizedComments = useMemo<NormalizedFeedback[]>(() => {
    return comments.map((comment: Feedback) => ({
      ...comment,
      id: nanoid(),
    }))
  }, [comments])
  ...
  if (error) {
    ...
    return (
      <div className="bg-rose-800 text-white p-4 rounded-md mx-auto max-w-[640px]">
        Error: {errorMessage}
        <p className="text-rose-700 text-center mt-2 p-2">
          Refresh the application or delete catch history in Browser
        </p>
      </div>
    )
  }
      ...
      <div className="px-4 sm:px-8 py-6">
        <div className="flex items-baseline gap-x-6">
          ...
          <div className="flex items-center justify-baseline gap-x-2">
            <button
              onClick={() => setOrderbyOld(false)}
              className={`p-1 w-20 rounded ${
                !orderbyOld ? 'border-b' : 'text-gray-400'
              }`}
            >
              Newest
            </button>
            <button
              onClick={() => setOrderbyOld(true)}
              className={`p-1 w-20 rounded ${
                orderbyOld ? 'border-b' : 'text-gray-400'
              }`}
            >
              ...
              <div className="flex items-start">
                ...
                <div className="flex-grow">
                  ...
                  <div className="flex justify-between mt-3">
                    ...
                    <button
                      onClick={() => handleUpvote(comment.id, comment.upvoteCount)}
                      className="hover:text-green-400 flex items-center text-sm font-Inter transition-all cursor-pointer hover:scale-105"
                    >
