  ...
  if (text.includes("#") && text.length > 5) {
   ...
   return(
    ...
    <div className="shadow border border-gray-800 w-[299px] sm:w-[400px] h-[150px] sm:h-[140px] rounded-md bg-zinc-700 mt-9 flex flex-col items-center justify-between p-2">
        <textarea value={text} onChange={(e)=>setText(e.target.value)} id="textarea" className="text-zinc-200 text-sm font-Inter focus:outline-none resize-none bg-zinc-700 w-full  flex-grow placeholder:text-zinc-400 sm:placeholder:text-base" maxLength="150"  placeholder="Enter Your feedback hear, remember to #hashtag the company!"></textarea>
        
        <div className="flex justify-between w-full px-2 items-center">
            <p id="counter" className="sm:text-sm text-xs font-Inter-Light ">150</p>
            <button id="submit-btn" onClick={()=>handleClickSubmit()} className="bg-white text-zinc-800 p-2 rounded-3xl font-Inter sm:text-base text-sm transition-all hover:scale-105">Submit</button>
        </div>
    </div>
   </div>
 </header>
