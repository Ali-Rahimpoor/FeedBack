...
const Header: React.FC = () => {
  ...
  const remainCharRef = useRef<number>(150)
  ...
  const handleTextChange = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (counterElement.current) {
      counterElement.current.textContent = remainCharRef.current.toString()
  ...
  const handleClickSubmit = async () => {
    if (text.includes('#') && text.length > 5) {
      ...
      setText('')
      if (text.length < 5) {
        toast.warning('Your comment must be more than 5 characters', {
          position: 'top-center',
        })
      } else {
        toast.warning('Your comment must have (#)! for Company name', {
          position: 'top-center',
        })
      }
      if (counterElement.current) {
  ...
  return (
    <header className="sm:mt-8 font-Karla">
      <div className="relative overflow-x-hidden w-[100%] sm:w-[640px] sm:rounded-t-lg bg-zinc-900 text-white pt-12 pb-6 mx-auto flex flex-col items-center justify-center">
        ...
        <h1 className="sm:text-4xl text-2xl font-Funnel">
          Give Feedback.{' '}
          <span className="font-Montserrat-Light text-base sm:text-xl">
            Publicly.
          </span>
        </h1>
        ...
        <div className="shadow border border-gray-800 w-[299px] sm:w-[400px] h-[150px] sm:h-[140px] rounded-md bg-zinc-700 mt-9 flex flex-col items-center justify-between p-2">
          <textarea
            ...
            id="textarea"
            className="text-zinc-200 text-sm font-Inter focus:outline-none resize-none bg-zinc-700 w-full flex-grow placeholder:text-zinc-400 sm:placeholder:text-base"
            maxLength={150}
            placeholder="Enter Your feedback hear, remember to #hashtag the company!"
          />
          ...
          <div className="flex justify-between w-full px-2 items-center">
            <p
              id="counter"
              ref={counterElement}
              className="sm:text-sm text-xs font-Inter-Light"
            >
              150
            </p>
            <button
              ...
              className="bg-white cursor-pointer hover:bg-zinc-300 text-zinc-800 p-2 rounded-3xl font-Inter sm:text-base text-sm transition-all hover:scale-105"
            >
              Submit
            </button>
          </div>
